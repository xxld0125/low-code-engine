...
- [ ] **7.1 [FE] 端到端流程验证：用户管理 CRUD**
  ...
  - [ ] 步骤：
    1. **页面搭建**：
       ...
       - 拖入 Table (绑定 users 表)。
    2. **新增功能**：
       - 拖入 Button (新建用户)，配置点击打开 `Modal_Create`。
       - 在 `Modal_Create` 中拖入 Form (绑定 users 表)，配置提交动作及成功后关闭模态框。
    3. **编辑功能 (核心缺口补充)**：
       - 在 Table 中添加“操作列”。
       - 在操作列拖入 Icon/Button (编辑)。
       - 配置编辑按钮点击动作：打开 `Modal_Edit` 并注入当前行数据 (`Row Context`)。
       - 在 `Modal_Edit` 中拖入 Form，验证表单是否自动回填了选中行的数据。
    4. **删除功能 (核心缺口补充)**：
       - 在操作列拖入 Icon/Button (删除)。
       - 配置删除按钮点击动作：触发 `deleteRecord` 并刷新表格。
    5. **预览与运行**：
       - 保存页面。
       - 进入 Runtime 预览模式。
    6. **功能验收**：
       - [ ] 测试新增数据，验证表格刷新及 Toast 成功提示。
       - [ ] 测试点击编辑，验证数据回显正确，修改后提交生效。
       - [ ] 测试删除数据，验证数据消失。

- [ ] **7.2 [FE] 交互与反馈验证**
  - [ ] 验证表单校验错误时的 UI 表现。
  - [ ] 验证操作成功/失败时的 Toast 提示 (Sonner)。
  - [ ] 验证模态框的遮罩层点击关闭行为。

- [ ] **7.3 [FE] 性能与优化**
  - [ ] 优化拖拽流畅度 (Dnd-kit 性能调优)。
  - [ ] 检查组件渲染性能 (避免 Table/Form 不必要的重渲染)。
  - [ ] 完善错误处理与提示。
