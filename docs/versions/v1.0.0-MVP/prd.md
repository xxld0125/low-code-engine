...
1.  **仪表盘 (Dashboard)**
    ...
    - **新建页面流程**：
      ...
      2. 弹出模态框或跳转至创建页。
      3. 输入页面基本信息：
         - **页面名称 (Name)**：必填，用于在 Dashboard 展示 (e.g., "用户管理")。
         - **页面路径 (Slug)**：系统自动生成 (UUID)，无需用户填写。
         - **页面描述 (Description)**：选填，用于补充说明页面用途。
      4. 提交后跳转至编辑器。
2.  **编辑器 (Editor)**
    - 路由：`/editor/:pageId`
...
- **布局**：三栏式布局。
  ...
  - **右侧**：属性面板。
    - **Props**: 组件特有属性。
    - **Style**: 布局与样式 (Display 仅限容器类组件)。
    - **Events**: 交互事件 (仅限 Button/Form)。
- **交互模式**：拖拽 + 代码增强。
- **画布操作**：
  - 选中组件。
  - 删除组件。
...
- **侧边栏拖拽**：
  - 拖拽预览（DragOverlay）居中对齐鼠标，避免偏移。
  - 原始组件保持原位不动，仅显示半透明状态。
  - 使用绿色标签（如 `TEXT_1 X`）作为拖拽跟随提示。
- **画布内拖拽**：
  - 被拖拽组件保持在原位，显示半透明占位符。
  - 不应用 `transform` 位移，避免下方组件意外移动（防止 Layout Shift）。
  - 通过绿色插入线（DropIndicator）明确指示放置位置。
- **视觉反馈统一**：
  - DragOverlay：跟随鼠标的绿色标签，显示组件类型/ID。
  - DropIndicator：目标位置的绿色横线或边框高亮。

#### 2.1.2 数据保护与提醒

- **未保存更改提醒**：
  - 当用户在编辑器中执行任何修改操作（添加/删除/移动/更新组件）后，系统标记页面为"有未保存更改"状态。
  - 用户尝试刷新或关闭页面时，浏览器弹出标准确认对话框，提示："确定要离开此页面吗？您所做的更改可能不会保存。"
  - 点击 Save 按钮后，清除"未保存更改"标记，允许正常刷新。

### 2.2 组件系统

**MVP 组件集**：

1.  **布局类**：容器 (Container) —— 支持 Flex/Grid 布局配置。
